...
async function checkProduct() {
  console.log('Проверка продукта 4T2EC002I.K в локальной базе данных Supabase...');
  
  try {
    const { data, error } = await supabase
      .from('products')
      .select('article, image')
      .eq('article', '4T2EC002I.K');
    ...
    if (error) {
      ...
      if (data && data.length > 0) {
        ...
        console.log('Продукт 4T2EC002I.K не найден в базе данных');
      }
    }
  } catch (err) {
